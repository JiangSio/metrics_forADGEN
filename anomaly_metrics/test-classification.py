...
def test(args,obj_name, model,anomaly_names):
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        df = pd.DataFrame([])
        path = args.result_save
        directory_path = os.path.dirname(path)
        os.makedirs(directory_path, exist_ok=True)
        if(os.path.exists(path)):
            df = pd.read_csv(path)
        new_row_df = pd.DataFrame([{"classname":obj_name,'Accuracy': str("%.4f"%(correct/len(label)))}])
...
if __name__=="__main__":
    ...
    parser.add_argument('--result_save', type=str,default="results/cla_result.csv")
