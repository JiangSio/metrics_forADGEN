...
def test(obj_names, mvtec_path, checkpoint_path,result_save_path):
    ...
    directory_path = os.path.dirname(result_save_path)
    os.makedirs(directory_path, exist_ok=True)
    with open(os.path.join(directory_path,"anomaly_loc_results.csv"), "w") as csvfile:
    ...
    for obj_name in obj_names:
        ...
        with open(os.path.join(directory_path,"anomaly_loc_results.csv"), "a") as csvfile:
            writer = csv.writer(csvfile)
    ...
    if(os.path.exists(path)):
        df = pd.read_csv(path)
    new_row_df = pd.DataFrame([{'AUC Image': str(np.mean(obj_auroc_image_list)), 
                                "AP Image":str(np.mean(obj_ap_image_list)),
                                'AUC Pixel': str(np.mean(obj_auroc_pixel_list))
                                ,'AP Pixel': str(np.mean(obj_ap_pixel_list)) ,
                                "Pro Pixel":str(np.mean(obj_pro_image_list))}])
