...
fake_data_root = "/data4/jiangtianjia/datasets/mvtec"
...
for cls in os.listdir(data_root):
    ...
    for anomaly in os.listdir(os.path.join(classpath,'ground_truth')):
        ...
        # 创建DataFrame
        df = pd.DataFrame([])
        path = "./avalidatable_analysis.csv"
        if(os.path.exists(path)):
            df = pd.read_csv(path)
        new_row_df = pd.DataFrame([{'anomaly': f'{cls}_{anomaly}', "IS↑":f"{inception_score:4f}",'ICLPIPS↑': f"{iclpips:4f}",'FID64↓': f"{fid64:4f}" ,"KID↓":f"{kid:4f}" ,"EMD↓":f"{emd:4f}", "KL↓":f"{kl:4f}", "JS↓":f"{js:4f}"}])
